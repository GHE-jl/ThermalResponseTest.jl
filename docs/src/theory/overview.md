...
recover with circulation but no heating. Interpreting the mean temperature allows recovering the ground and borehole
...
1. **First-order approximation** linearizes `g(t)` in its long-time limit,
...
- Use for **FOA** for a quick estimate or a quick check with no dependency on optimizer
  convergence, it is the standard, ubiquitous approach for conventional single-borehole tests.
- Use for **model inversion** when the FOA's validity assumptions don't hold well (e.g. a short
...
- Running both and comparing `k`/`Rbₑ` is a reasonable check either way, since the two
...
All fitting functions expect `t` measured from the start of **heating** (i.e., `t = 0` the instant the
heater turns on), and strictly positive and uniformly spaced values (for the model-inversion methods, whose forward model is an FFT convolution). [`decompose_trt`](@ref) enforces this automatically via `:t_rel`.