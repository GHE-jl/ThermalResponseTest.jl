...
- **Non-uniform time steps.** If any step drifts from the median by more than `dt_tol` (default
  0.1 s), the whole series is PCHIP-interpolated onto a uniform grid at the median step.
...
threshold (default 100 W). `heating` and `recovery` each carry a `:t_rel` column, a time rebased so
`t_rel = 0` at the instant heating starts, which is what every `TRTDataset`-based fitting method
in [First-order approximation](@ref) actually consumes. `data` is the
full test (both phases), used by the [Model inversion](@ref) methods, which fit the whole signal at once
rather than one phase at a time.

`TRTDataset` assumes exactly one heating phase followed by one recovery phase, representing a **conventional**
TRT. An unconventional test (several on/off cycles) does not fit that shape. Such a test should be interpreted with the
